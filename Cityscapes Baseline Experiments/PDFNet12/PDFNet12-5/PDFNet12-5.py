...
class PDFNet(nn.Module):
  def __init__(self):
    ...
    self.s3 = nn.Sequential(nn.Conv2d(544, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
    ...
    self.s4 = nn.Sequential(nn.Conv2d(928, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
    ...
    self.s5 = nn.Sequential(nn.Conv2d(1312, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
...
checkpoint_dir = "./PDFNet12-5/"
best_model_dir = "./PDFNet12-5/"
...
ckp_path = "./PDFNet12-5/checkpoint.pt"
...
def train_val(model, params):
    ...
    #best_model_wts = copy.deepcopy(torch.load("./PDFNet12-5/PDFNet12-5_weights.pt"))
    ...
    for epoch in range(num_epochs):
        current_lr=get_lr(opt)
        o = open('./PDFNet12-5/PDFNet12-5.txt','a')
...
import os
path2models= "./PDFNet12-5/PDFNet12-5_"
...
o = open('./PDFNet12-5/PDFNet12-5-time.txt','a')
...
plt.legend()
plt.savefig('./PDFNet12-5/PDFNet12-5.png', dpi = 300)
...
with open('./PDFNet12-5/PDFNet12-5.csv', 'w') as f:
    writer = csv.writer(f)
