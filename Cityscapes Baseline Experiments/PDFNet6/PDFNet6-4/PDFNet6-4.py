...
class PDFNet(nn.Module):
  def __init__(self):
    ...
    self.s3 = nn.Sequential(nn.Conv2d(352, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
    ...
    self.s4 = nn.Sequential(nn.Conv2d(544, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
    ...
    self.s5 = nn.Sequential(nn.Conv2d(736, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
...
checkpoint_dir = "./PDFNet6-4/"
best_model_dir = "./PDFNet6-4/"
...
ckp_path = "./PDFNet6-4/checkpoint.pt"
...
def train_val(model, params):
    ...
    #best_model_wts = copy.deepcopy(torch.load("./PDFNet6-4/PDFNet6-4_weights.pt"))
    ...
    for epoch in range(num_epochs):
        current_lr=get_lr(opt)
        o = open('./PDFNet6-4/PDFNet6-4.txt','a')
...
import os
path2models= "./PDFNet6-4/PDFNet6-4_"
...
o = open('./PDFNet6-4/PDFNet6-4-time.txt','a')
...
plt.legend()
plt.savefig('./PDFNet6-4/PDFNet6-4.png', dpi = 300)
...
with open('./PDFNet6-4/PDFNet6-4.csv', 'w') as f:
    writer = csv.writer(f)
