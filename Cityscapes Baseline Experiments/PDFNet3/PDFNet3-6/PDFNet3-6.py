...
class PDFNet(nn.Module):
  def __init__(self):
    ...
    self.s3 = nn.Sequential(nn.Conv2d(256, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
    ...
    self.s4 = nn.Sequential(nn.Conv2d(352, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
    ...
    self.s5 = nn.Sequential(nn.Conv2d(448, 20, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(20),
                            nn.ReLU(inplace=True))
...
checkpoint_dir = "./PDFNet3-6/"
best_model_dir = "./PDFNet3-6/"
...
#ckp_path = "./PDFNet3-6/checkpoint.pt"
...
def train_val(model, params):
    ...
    #best_model_wts = copy.deepcopy(torch.load("./PDFNet3-6/PDFNet3-6_weights.pt"))
    ...
    for epoch in range(num_epochs):
        current_lr=get_lr(opt)
        o = open('./PDFNet3-6/PDFNet3-6.txt','a')
...
import os
path2models= "./PDFNet3-6/PDFNet3-6_"
...
o = open('./PDFNet3-6/PDFNet3-6-time.txt','a')
...
plt.legend()
plt.savefig('./PDFNet3-6/PDFNet3-6.png', dpi = 300)
...
with open('./PDFNet3-6/PDFNet3-6.csv', 'w') as f:
    writer = csv.writer(f)
