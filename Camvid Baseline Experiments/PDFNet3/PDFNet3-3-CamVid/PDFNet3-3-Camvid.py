...
class PDFNet(nn.Module):
  def __init__(self):
    ...
    self.s3 = nn.Sequential(nn.Conv2d(256, 12, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(12),
                            nn.ReLU(inplace=True))
    
    self.glance41 = look(in_filters=256, dilation_rate=1)
    self.glance42 = look(in_filters=288, dilation_rate=2)
    self.glance43 = look(in_filters=320, dilation_rate=3)


  

    self.s4 = nn.Sequential(nn.Conv2d(352, 12, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(12),
                            nn.ReLU(inplace=True))
    

    self.glance51 = look(in_filters=352, dilation_rate=1)
    self.glance52 = look(in_filters=384, dilation_rate=2)
    self.glance53 = look(in_filters=416, dilation_rate=3)




    self.s5 = nn.Sequential(nn.Conv2d(448, 12, kernel_size=1, stride =1,bias=False),
                            nn.BatchNorm2d(12),
                            nn.ReLU(inplace=True))
...
checkpoint_dir = "./PDFNet3-3-CamVid/"
best_model_dir = "./PDFNet3-3-CamVid/"
...
#ckp_path = "./PDFNet3-3-CamVid/checkpoint.pt"
...
def train_val(model, params):
    ...
    #best_model_wts = copy.deepcopy(torch.load("./PDFNet3-3-CamVid/PDFNet3-3_weights.pt"))

    #best_loss= min(loss_history["val"])
    best_loss=float('inf')
    o = open('./PDFNet3-3-CamVid/PDFNet3-3.txt','a')
...
import os
path2models= "./PDFNet3-3-CamVid/PDFNet3-3_"
...
o = open('./PDFNet3-3-CamVid/PDFNet3-3-time.txt','w')
...
plt.legend()
plt.savefig('./PDFNet3-3-CamVid/PDFNet3-3.png', dpi = 300)
...
print(iou_sum/101)
o = open('./PDFNet3-3-CamVid/PDFNet3-3_IOU_val.txt','w')
...
print(i_sum/233)
o = open('./PDFNet3-3-CamVid/PDFNet3-3_IOU_test.txt','w')
...
with open('./PDFNet3-3-CamVid/PDFNet3-3-plot.csv', 'w') as f:
    writer = csv.writer(f)
